...
本地开源网站定制器 PhilWeb Studio。React 19 + Vite + Tailwind，纯静态产物，无后端。
...
templates/starter-content/  # 中性示例内容，content:reset 的默认底稿
...
6. **成品必须隔离**：用户部署的是 `npm run export` 生成的 ZIP，不是源码目录或普通开发目录。
7. **改完必须验证**：`npm run content:check`（内容）→ `npm run verify`（全量）→ `npm run build` → `npm run export`。
...
npm run export            # 校验并生成只含静态文件的网站 ZIP
...
- `scripts/export-site.mjs` 在临时目录构建并压缩成品，不覆盖工作区 `dist/`，导出 ZIP 不含源码、content 或 Studio。
